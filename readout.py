...
class ActionReadout(nn.Module):
    ...
    def act_batch(self, dn_activity_dicts):
        """
        Batched version of act: takes a list of N {dn_type: value} dicts,
        returns (actions, log_probs) where actions is a plain list of N
        ints and log_probs is a tensor of shape (N,), one gradient-
        tracked log-probability per environment.
        """
        x = torch.stack([self.activity_dict_to_tensor(d) for d in dn_activity_dicts])
        logits = self.linear(x)  # (N, NUM_ACTIONS)
        distribution = torch.distributions.Categorical(logits=logits)
        actions = distribution.sample()  # (N,)
        log_probs = distribution.log_prob(actions)  # (N,), still tracks gradients
        return actions.tolist(), log_probs
